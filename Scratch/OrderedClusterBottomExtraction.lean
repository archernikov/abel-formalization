...
private theorem test_extract_under_coefficient_equiv
...
theorem test_exists_current_certificate
    {R : Type u} [CommRing R]
    {m : ℕ} {time : ℕ → Fin m → ℝ}
...
theorem test_exists_bottom_certificate_height
    {R : Type u} [CommRing R]
    {m : ℕ} {time : ℕ → Fin m → ℝ}
    {data : RepresentativeClusterSubsequence time}
    {higher : ℕ}
  ...
  obtain ⟨nextIdeal, tail, certificate, _⟩ :=
    test_exists_current_certificate descent hcount
...
theorem test_exists_bottom_timePolynomial
  ...
  obtain ⟨nextIdeal, tail, certificate, htime⟩ :=
    test_exists_bottom_certificate_height descent hcount p hheight
  ...
  obtain ⟨P, hP, hPembedded, hPmem⟩ :=
    test_extract_under_coefficient_equiv p d e0R
