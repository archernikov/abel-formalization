...
/-- Each member of the finite exceptional datum family has a concrete
finite tower over the common base. -/
theorem exists_tower_restrictedPairMergeExceptionalTowerDatum
    (A : ℝ → ℝ) {m p a : ℕ} (D : RestrictedBox p)
    (representative : ι → Fin ((m + 1) + 1))
    (offset : ι → RestrictedBox.analyticNearClosedBoxSubalgebra D)
    (K k : ℕ) (i : Fin ((m + 1) + 1)) (j : Fin (m + 1))
    (S : Finset (ι × ℕ))
    (h : RestrictedPairMergeExceptionalTowerDatumIndex
      (restrictedPairMergeExceptionalOffsetSupport representative i j S).card
      (restrictedPairMergeExceptionalOrderBound S)) :
    ...
    ∃ l : ℕ, ∃ T : RestrictedExpressionTower
      ...
      restrictedPairMergeExceptionalTowerDatum
        (a := a) A D representative offset K k i j S h ∈ T.level l := by
  dsimp only
  let N := (restrictedPairMergeExceptionalOffsetSupport representative i j S).card
  let G := restrictedPairMergeExceptionalCommonJetGenerators
    (a := a) A D representative offset i j S
  ...
  · cases b with
    | false =>
        let u : RestrictedSource (m + 1) ((p + 1) + N) a → ℝ :=
          fun z ↦ z.1.1 j + z.1.2 (Fin.castAdd N (Fin.last p))
        have hu : u ∈ base := base.add_mem
          (restrictedSCoordinate_mem_base _ G j)
          (restrictedWCoordinate_mem_base _ G (Fin.castAdd N (Fin.last p)))
        let T := iterateETower base u (K + k) hu
        refine ⟨K + k, T, ?_⟩
        exact iterateE_mem_iterateETower_terminal base u (K + k) hu
    | true =>
        let u : RestrictedSource (m + 1) ((p + 1) + N) a → ℝ :=
          fun z ↦ z.1.1 j
        have hu : u ∈ base := restrictedSCoordinate_mem_base _ G j
        let T := iterateETower base u K hu
        refine ⟨K, T, ?_⟩
        exact iterateE_mem_iterateETower_terminal base u K hu
  · rcases h with t | h
    · obtain ⟨T₀, hT₀⟩ :=
    ...
    · rcases h with ⟨⟨t, r⟩, b⟩
      ...
      obtain ⟨T₀, hden, hnum⟩ :=
        exists_tower_restrictedPairMergeExceptionalDerivativeData
          (a := a) A D representative i j S t d r.val
      let G₀ := restrictedPairMergeExceptionalGraphAbelJetGenerators
        (a := a) A D representative i j S
      let base₀ := restrictedExpressionBase
        (restrictedPairMergeExceptionalGraphBox D N) G₀
      ...
      refine ⟨d, T, ?_⟩
      cases b with
      | false => exact exponentialLevels_mono_base hbase T₀.exponent d hden
      | true => exact exponentialLevels_mono_base hbase T₀.exponent d hnum

/-- All finite exceptional data for one compressed system belongs to one
