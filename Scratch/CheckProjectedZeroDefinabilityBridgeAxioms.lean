...
run_elab do
  ...
  let declarations ← env.constants.foldM (init := #[]) fun names name info => do
    if let some moduleIdx := env.getModuleIdxFor? name then
      if env.header.moduleNames[moduleIdx.toNat]! ==
          `AbelFormalization.ProjectedZeroDefinabilityBridge then
  ...
  logInfo m!"All {declarations.size} definability-bridge declarations pass, including {theoremCount} theorems."
