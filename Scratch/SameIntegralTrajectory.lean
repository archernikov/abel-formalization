...
namespace AbelFormalization.Scratch
...
theorem isIntegralCurveOn_Ioo_eqOn_of_contDiff
  ...
  · rw [isOpen_iff_mem_nhds]
    ...
    have hlocal : γ =ᶠ[𝓝 t] δ :=
      isIntegralCurveAt_eventuallyEq_of_contDiffAt hV.contDiffAt
...
theorem isIntegralCurveOn_piecewise_Ioo
  ...
  · have ht' := ht
    ...
    rw [Filter.eventuallyEq_iff_exists_mem]
    refine ⟨Ioo a' b', ?_,
      eqOn_piecewise_of_isIntegralCurveOn_Ioo hV hγ hδ ht₀ heq⟩
    rw [(isOpen_Ioo.union isOpen_Ioo).nhdsWithin_eq ht']
...
def trans (hV : ContDiff ℝ 1 V)
  ...
  have hη : IsIntegralCurveOn η (fun _ ↦ V)
      (Ioo hxy.lower hxy.upper ∪ Ioo a' b') := by
    exact isIntegralCurveOn_piecewise_Ioo hV hxy.isIntegral hδ halign_mem halign
  have hη_eq_δ : EqOn η δ (Ioo a' b') := by
    exact eqOn_piecewise_of_isIntegralCurveOn_Ioo
      hV hxy.isIntegral hδ halign_mem halign
