...
/-- The uniform restricted level-zero theorem therefore gives 0-regularity
of the concrete localized Abel family. -/
theorem IsAbel.isZeroRegular_abelGeometricFamily_of_restrictedBase
    {A : ℝ → ℝ} (hA : IsAbel A)
    (hbase : ∀ m, RestrictedBaseRegularZeroFiniteForRepresentativeCount A m) :
    IsZeroRegularFunctionFamily (abelGeometricFamily A) :=
  hA.isZeroRegular_abelGeometricFamily_of_numerator
    (hA.abelNumeratorRegularZeroFinite_of_restrictedBase hbase)
