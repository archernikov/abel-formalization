...
This file proves the full representative-count-zero statement `P(0,0)` from
the analytic-germ elimination calculation.
...
The final construction applies the restricted paper-rank elimination theorem
after translating a chosen box point to the analytic-germ origin.  The
contracted ideal isolates the parameter coordinates, while the full
unit-augmented ideal and `analyticGerm_polynomial_monic_extraction` leave only
finitely many auxiliary tuples.
-/
...
theorem finite_specialized_commonZero_of_analyticGerm_height_fintype
  ...
  have hJ : (p + Fintype.card σ : ℕ∞) ≤ J.height := by
    dsimp only [J]
    have he : (I.map phi.toRingHom).height = I.height :=
...
theorem finite_specialized_commonZero_unitAugmentedJacobian
  ...
  have hcard' : (p + Fintype.card (Option σ) : ℕ∞) = (n + 1 : ℕ∞) := by
...
/-- The representative-count-zero case at every finite exponential-list
level.  This is the complete inner-induction base used in the manuscript's
outer induction. -/
theorem IsAbel.restrictedRegularZeroFiniteForRepresentativeCount_zero_all
    (hA : IsAbel A) :
    ∀ level, RestrictedRegularZeroFiniteForRepresentativeCount A 0 level :=
  hA.restrictedRegularZeroFiniteForRepresentativeCount_all_of_base
