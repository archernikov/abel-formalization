...
/-- Every ordinary exponential used to advance the `E`-orbit occurs in the
terminal algebra of the orbit tower. -/
theorem iterateE_exp_mem_iterateETower_terminal
    (base : Subalgebra ℝ (X → ℝ)) (f : X → ℝ) (n : ℕ)
    (hf : f ∈ base) {d : ℕ} (hd : d < n) :
    (fun x ↦ Real.exp (E^[d] (f x))) ∈
      (iterateETower base f n hf).level n := by
  let T := iterateETower base f n hf
  have hgen := T.generator_mem_level_of_lt (⟨d, hd⟩ : Fin n) hd
  change (fun x ↦ Real.exp (E^[d] (f x))) ∈ T.level n
  convert hgen using 1
  funext x
  rfl

/-- A finite `E`-orbit tower contains every common denominator needed to
transport a derivative backwards through its full length. -/
theorem iteratedAbelDerivativeDenominator_mem_iterateETower_terminal
    (base : Subalgebra ℝ (X → ℝ)) (f : X → ℝ) (d r : ℕ)
    (hf : f ∈ base) :
    (fun x ↦ iteratedAbelDerivativeDenominator d r (f x)) ∈
      (iterateETower base f d hf).level d := by
  apply iteratedAbelDerivativeDenominator_mem_subalgebra_bounded_of_exp_orbit
    ((iterateETower base f d hf).level d) f d
  intro e he
  exact iterateE_exp_mem_iterateETower_terminal base f d hf he

/-- If the base contains the original Abel jets, the same orbit tower
contains every denominator-cleared transported numerator. -/
theorem iteratedAbelDerivativeNumerator_mem_iterateETower_terminal
    (base : Subalgebra ℝ (X → ℝ)) (A : ℝ → ℝ)
    (f : X → ℝ) (d r : ℕ) (hf : f ∈ base)
    (hjet : ∀ q : ℕ, (fun x ↦ iteratedDeriv q A (f x)) ∈ base) :
    (fun x ↦ iteratedAbelDerivativeNumerator A d r (f x)) ∈
      (iterateETower base f d hf).level d := by
  apply iteratedAbelDerivativeNumerator_mem_subalgebra_bounded_of_exp_orbit
    ((iterateETower base f d hf).level d) A f
  · intro q
    exact (iterateETower base f d hf).base_mem_level (hjet q) d
  · intro e he
    exact iterateE_exp_mem_iterateETower_terminal base f d hf he
